...
/*
   @author  yuan
   @time    2018/11/25 18:17
*/
@WebServlet(name = "InputCodeCheck")
public class InputCodeCheck extends HttpServlet {
    // 宽和高定义
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        String baseLetterDown = "ABCDEFGHIJKLMNOPQRSTUVWXYZ";
        ...
        String allChar = baseNum+baseLetterUp;
        allChar+=baseLetterDown;
        allChar+=baseChineseChar;
        Graphics2D g2D = (Graphics2D)g;
        StringBuffer strBuf = new StringBuffer();
        for(int i=0;i<4;i++){
            int degree = new Random().nextInt() % 30;
            String ch = allChar.charAt(new Random().nextInt(allChar.length()))+"";
            g2D.rotate(degree*Math.PI/180,5+i*90,HEIGHT/2);
            g2D.drawString(ch,10+i*WIDTH/4,HEIGHT/2);
            g2D.rotate(-degree*Math.PI/180,5+i*90,HEIGHT/2);
            strBuf.append(ch);
        }
        g.setColor(new Color(0x7d7d7d));
        for (int i = 0; i < 5; i++) {
        ...
        request.getSession().setAttribute("CheckCode",strBuf.toString());
